...
export function ArticleChatPage() {
  ...
  if (article.isLoading) return <div className="mx-auto max-w-3xl"><Spinner label="Loading article…" /></div>
  if (article.isError || !article.data) return <div className="mx-auto max-w-3xl"><Link to="/latest" className="text-sm font-medium text-brand-700 hover:underline">← Back to latest news</Link><div className="mt-6"><ErrorState error={article.error} onRetry={() => article.refetch()} /></div></div>
